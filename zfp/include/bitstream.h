...
extern "C"
{
    ...
    bitstream *bitstream_open(void *buffer, size_t bytes);

    /* close and deallocate bit stream */
    void bitstream_close(bitstream *stream);
    ...
    size_t bitstream_size(const bitstream *stream);
    ...
    size_t bitstream_flush(bitstream *stream);
