...
compress_init_par(zfp_stream *stream, const zfp_field *field, size_t chunks, size_t blocks)
{
  ...
  for (chunk = 0; chunk < chunks; chunk++)
  {
    ...
    void *buffer = copy ? malloc(size) : (uchar *)stream_data(stream->stream) + bitstream_size(stream->stream) + block * (stream->maxbits / CHAR_BIT);
    if (!buffer)
      break;
