...
SRC = $(ZFP_SRC) src/webql.ispc src/json.c src/ini.c src/mongoose.c src/mjson.c src/hash_table.c src/cluster.c src/http.c src/main.c
OBJ := $(SRC:.f90=.o)
OBJ := $(OBJ:.c=.o)
OBJ := $(OBJ:.ispc=.o)
DEP = $(OBJ:%.o=%.d)
...
LIBS = -L/usr/local/lib -lmicrohttpd `pkg-config --libs glib-2.0` `pkg-config --libs libcpuid` -llz4 -L/usr/local/lib64 -lcfitsio -lsqlite3 -lz -pthread -lzmq -lczmq
# -lzfp before cfitsio
