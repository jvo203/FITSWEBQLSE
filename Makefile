...
SRC = $(ZFP_SRC) src/webql.ispc src/json.c src/ini.c src/mongoose.c src/mjson.c src/histogram.c src/hash_table.c src/psrs_sort.c src/cluster.c src/http.c src/ws.c src/face.f90 src/logging.f90 src/lttb.f90 src/zfp_array.f90 src/classifier.f90 src/fits.f90 src/main.c
OBJ := $(SRC:.f90=.o)
OBJ := $(OBJ:.c=.o)
OBJ := $(OBJ:.ispc=.o)
DEP = $(OBJ:%.o=%.d)
