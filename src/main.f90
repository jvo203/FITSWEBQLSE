program test_caf
    ...
    character(len=256) :: filename[*] ! scalar coarray, one "name" for each image.
    ! Note: "filename" is the local variable while "filename[<index>]" accesses the
    ! variable in a specific image; "filename[this_image()]" is the same as "filename".
    real(c_float) :: dmin[*], dmax[*]
    logical bSuccess[*]

    ! call MPI_INIT(ierror)
    call MPI_COMM_SIZE(MPI_COMM_WORLD, size, ierror)
    call MPI_COMM_RANK(MPI_COMM_WORLD, rank, ierror)
    print *, 'node', rank, '/', size

    ! Interact with the user on Image 1; execution for all others pass by.
    if (this_image() == 1) then
        filename = '../FITSWebQL/.cache/ALMA01003454.fits'
        ! write (*, '(a)', advance='no') 'Enter the FITS file path: '
        ! read (*, '(a)') filename
    end if

    ! Distribute information to all images
    call co_broadcast(filename, source_image=1)

    ! I/O from all images, executing in any order, but each record written is intact.
    ! '(i0, 3a)'
    ! write (*, *) 'image # ', this_image(), ', loading ', trim(filename)

    call read_fits_file(filename, dmin, dmax, bSuccess)
    call co_reduce(bSuccess, myand)

    if (this_image() == 1) then
        print *, 'image # ', this_image(), ', reading ', trim(filename), ': bSuccess ', bSuccess
    end if
