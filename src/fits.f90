module fits
   ...
   integer(kind=4), parameter :: MAGIC_NUMBER = 20251021
   ...
   interface
      ...
      ! export void viewport_moment_map_1_rect(uniform struct fixed_block_t compressed[], uniform int width, uniform int height, uniform float pmin, uniform float pmax, uniform float view_I[], uniform float view_Iv[], uniform bool view_mask[], uniform int stride, uniform int x1, uniform int x2, uniform int y1, uniform int y2, uniform int horizontal, uniform int vertical, uniform double velocity)
      subroutine viewport_moment_map_1_rect(compressed, width, height, pmin, pmax, view_I, view_Iv, view_mask, stride,&
      ...
      ! void write_ws_polarisation(websocket_session *session, const int *seq_id, const float *timestamp, const float *elapsed, int width, int height, int pol_target, const float *restrict intensity, const float *restrict angle, int precision);
      subroutine write_ws_polarisation(session, seq_id, timestamp, elapsed, width, height, pol_target,&
      & intensity, angle, precision) BIND(C, name='write_ws_polarisation')
         use, intrinsic :: ISO_C_BINDING
         implicit none
         ...
         integer(c_int), intent(in) :: seq_id
         real(c_float), intent(in) :: timestamp, elapsed
         integer(c_int), value, intent(in) :: width, height, pol_target
         type(C_PTR), value :: intensity, angle
...
contains
   ...
   subroutine load_dataset(item, cache, root, bSuccess)
      ...
      if ((N .eq. depth*max_planes) .and. (N .gt. 0)) then
         if (allocated(item%frame_min)) deallocate (item%frame_min)
         allocate (item%frame_min(depth, max_planes))
      ...
      if ((N .eq. depth*max_planes) .and. (N .gt. 0)) then
         ...
         allocate (item%frame_max(depth, max_planes))
      ...
      if ((N .eq. depth*max_planes) .and. (N .gt. 0)) then
         ...
         allocate (item%frame_median(depth, max_planes))
      ...
      if ((dims(1)*dims(2)*max_planes .eq. N) .and. (N .gt. 0)) then
      ...
      if ((N .eq. depth*max_planes) .and. (N .gt. 0)) then
         ...
         allocate (item%mean_spectrum(depth, max_planes))
      ...
      if ((N .eq. depth*max_planes) .and. (N .gt. 0)) then
         ...
         allocate (item%integrated_spectrum(depth, max_planes))
...
110         block
      ...
      ! Subaru HSC also uses 'DATE-AVG' for the observation mid-point, so try that first
...
200   call ftclos(unit, status)
   ...
   subroutine make_image_statistics(item, width, height, pixels, mask, outhist, tone, plane)
      ...
      black = hist_quantile(data, pmin, pmax, 0.05)
      white = hist_quantile(data, pmin, pmax, 0.95)
      ...
      print *, 'plane:', plane, 'flux: ', tone%flux, ', black = ', black, ', white = ', white,&
      & ', sensitivity = ', sensitivity, ', ratio sensitivity = ', ratio_sensitivity
   ...
   subroutine image_spectrum_request(ptr, width, height, precision, fetch_data, fd) bind(C)
      ...
      if (fetch_data .eq. 1) then
         ...
         ! mean spectrum
         if (allocated(item%mean_spectrum)) then
            call write_spectrum(fd, size(item%mean_spectrum, 2), c_loc(item%mean_spectrum), size(item%mean_spectrum, 1),&
            & ZFP_HIGH_PRECISION)
         end if

         ! integrated spectrum
         if (allocated(item%integrated_spectrum)) then
            call write_spectrum(fd, size(item%integrated_spectrum, 2), c_loc(item%integrated_spectrum),&
            & size(item%integrated_spectrum, 1), ZFP_HIGH_PRECISION)
         end if
      end if
...
8000  if (req%fd .ne. -1) call close_pipe(req%fd)
      ...
      if (req%image) then
         ...
         if (native_size .gt. viewport_size) then
            ...
            do k = 1, max_planes
               ...
               ! launch a pthread to resize pixels
               task_pid(k) = my_pthread_create(start_routine=c_funloc(launch_resize_task),&
               & arg=c_loc(pixels_task(k)), rc=task_rc(k))
            end do
   ...
   subroutine de_window(b, w, gamma, mu, theta, view, mask, max_iter)
      ...
      do iter = 1, max_iter
         ! evaluate the population
         !$omp parallel shared(pop, view, mask, w) private(i, cost, mu0, sigma0, theta0, genome)&
         !$omp& NUM_THREADS(max_threads)
      ...
      ! print the final best solution
      if (pop%best_idx .ne. 0) then
         print *, "de_window best cost:", pop%best_cost, "best genotype:", pop%best(pop%best_idx)%genotype
   ...
   subroutine trace_hds_spectrum(x1, y1, x2, y2, x0, y0, len, theta, view_pixels, view_mask, spectrum, mask)
      ...
      print *, 'outspec:', spectrum(1:10)
      print *, 'outmask:', mask(1:10)
   ...
   subroutine realtime_viewport_request(item, req)
      ...
      if (native_size .gt. viewport_size) then
         ...
         do i = 1, max_planes
            ...
            ! launch a pthread to resize pixels
            task_pid(i) = my_pthread_create(start_routine=c_funloc(launch_resize_task),&
            & arg=c_loc(pixels_task(i)), rc=task_rc(i))
         end do
   ...
   type(C_PTR) function DownsizeJSONPolarization(hdr, fits_width, fits_height, pixels, mask, width, height, beam)
      ...
      call add_json_integer(json, 'range'//c_null_char, range)
   ...
   &pol_ymin, pol_xmax, pol_ymax, pol_target, pol_intensity, pol_angle)
      ...
      do j = ymin, ymax, range
         do i = xmin, xmax, range
            ! print *, 'DownsizePolarizationSIMD: i:', i, 'j:', j, 'range:', range, 'xmax:', xmax, 'ymax:', ymax
...
10    format(a, i0, a, i0, a, i0, a, i0, a, i0, a, i0, a)
   ...
   recursive subroutine video_request(user) BIND(C, name='video_request')
      ...
      if (req%mask) then
         ...
         call get_video_frame(item, req%frame, req%plane, req%fill, tone, pixels, mask,&
         & req%width, req%height, req%downsize)
      else
         call get_composite_video_frame(item, req%frame, req%plane, req%fill, tone, pixels,&
         & req%width, req%height, req%downsize, 1)
      end if
...
9000  nullify (flux)
   ...
   subroutine get_composite_video_frame(item, frame, plane, fill, tone, dst_pixels, dst_width, dst_height, downsize, va_count)
      use, intrinsic :: iso_c_binding
      implicit none
   ...
   recursive subroutine ws_pv_request(user) BIND(C, name='ws_pv_request')
      ...
      if (scale .lt. 1.0) then
         img_width = floor(scale*npoints)
         img_height = floor(scale*length)
      ...
      if (rc .ne. 0) then
         ...
         call write_ws_pv_diagram(req%session, req%seq_id, req%timestamp, img_width, img_height, ZFP_PV_PRECISION,&
         &c_loc(pixels), pmean, pstd, pmin, pmax, 1, npoints, v1, v2, x1, y1, x2, y2)
   ...
   subroutine get_pv_diagram(item, req, pixels, width, height, pmin, pmax, pmean, pstd, npoints, v1, v2)
      ...
      if (scale .lt. 1.0) then
         ...
         if (scale .gt. 0.2) then
            call resizeLanczos(c_loc(pv), npoints, length, c_loc(pixels), width, height, 3)
         else
            call resizeSuper(c_loc(pv), npoints, length, c_loc(pixels), width, height)
         end if
      else
         width = npoints
         height = length
   ...
   end subroutine ws_composite_pv_request
   ...
   recursive subroutine ws_image_spectrum_request(user) BIND(C, name='ws_image_spectrum_request')
      ...
      real(c_float) :: dmad(4), dmadP(4), dmadN(4)
      ...
      if (req%fd .ne. -1) then
         ...
         if (item%is_stokes .and. max_planes .ge. 3) then
            ...
            & c_loc(intensity), c_loc(angle), precision)
         else
            ! send dummy polarisation information
