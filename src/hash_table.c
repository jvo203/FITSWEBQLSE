...
{
    size_t frame_offset = (size_t)frame * (size_t)no_planes * frame_size;
    ssize_t bytes_read = pread(fd, dst, frame_size, frame_offset + (size_t)plane * frame_size);
...
int read_array(const char *file, void *dst, size_t frame_size)
{
    ...
    int stat = read_frame(fd, dst, 0, 0, frame_size, 1);
