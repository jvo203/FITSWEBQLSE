...
static enum MHD_Result streaming_gzip_response(struct MHD_Connection *connection, char *buf, size_t len)
{
    ...
    CALL_ZLIB(deflateInit2(&(req->z), Z_DEFAULT_COMPRESSION, Z_DEFLATED, _windowBits | GZIP_ENCODING, 9, Z_DEFAULT_STRATEGY));
...
static enum MHD_Result execute_alma(struct MHD_Connection *connection, char **va_list, int va_count, int composite, char *root)
{
    ...
    g_string_append(html, "<link rel=\"stylesheet\" href=\"https://cdn.jsdelivr.net/gh/jvo203/fits_web_ql/htdocs/fitswebql/css/fontawesome.min.css\"/>\n");
    g_string_append(html, "<link rel=\"stylesheet\" href=\"https://cdn.jsdelivr.net/gh/jvo203/fits_web_ql/htdocs/fitswebql/css/solid.min.css\"/>\n");
    g_string_append(html, "<link rel=\"stylesheet\" href=\"https://cdn.jsdelivr.net/gh/jvo203/fits_web_ql/htdocs/fitswebql/css/regular.min.css\"/>\n");
    ...
    if (options.local)
    {
        ...
        g_string_append(html, "<script id=\"rgba-shader\" type=\"x-shader/x-vertex\">\n");
#ifdef SHARE
        include_file(html, SHARE "/htdocs/fitswebql/rgba-shader.frag");
#else
        include_file(html, "htdocs/fitswebql/rgba-shader.frag");
#endif
        g_string_append(html, "</script>\n");
        ...
        g_string_append(html, "<script id=\"z-score-shader\" type=\"x-shader/x-vertex\">\n");
#ifdef SHARE
        include_file(html, SHARE "/htdocs/fitswebql/z-score-shader.frag");
#else
        include_file(html, "htdocs/fitswebql/z-score-shader.frag");
#endif
        g_string_append(html, "</script>\n");

        g_string_append(html, "<script id=\"z-score-composite-shader\" type=\"x-shader/x-vertex\">\n");
#ifdef SHARE
        include_file(html, SHARE "/htdocs/fitswebql/z-score-composite-shader.frag");
#else
        include_file(html, "htdocs/fitswebql/z-score-composite-shader.frag");
#endif
        g_string_append(html, "</script>\n");
...
void fetch_channel_range(char *root, char *datasetid, int len, int *start, int *end, int *no_planes, int stride, int *status, float *frame_min, float *frame_max, float *frame_median, float *mean_spectrum, float *integrated_spectrum)
{
    ...
    if (progress > 0)
    {
        ...
        post_size += 5 * (size_t)(*no_planes) * (size_t)(progress) * sizeof(float);
    }
...
void write_spectrum(int fd, int no_planes, const float *spectrum, int n, int precision)
{
    ...
    if (compressed != NULL)
    {
        ...
        if (stream != NULL)
        {
            ...
            printf("[C] float array size: %zu bytes [no. plane(s): %d], compressed: %zu bytes\n", ny * length * sizeof(float), no_planes, zfpsize);
...
void write_image_spectrum(int fd, int no_planes, struct image_tone_mapping_type *tone, int width, int height, int precision, const float *restrict pixels, const bool *restrict mask)
{
    ...
    for (int i = 0; i < no_planes; i++)
        printf("[C] write_image_spectrum fd: %d, width: %d, height: %d; plane %d tone mapping flux: %s, pmin: %.10f, pmax: %.10f, pmedian: %.10f, black: %f, white: %f, sensitivity: %f, ratio_sensitivity: %f\n", fd, width, height, (i + 1), tone[i].flux, tone[i].pmin, tone[i].pmax, tone[i].pmedian, tone[i].black, tone[i].white, tone[i].sensitivity, tone[i].ratio_sensitivity);
